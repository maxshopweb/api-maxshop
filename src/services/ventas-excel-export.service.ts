/**
 * Servicio compartido: mapeo venta → filas Excel ERP (mismo formato que Ventas.xlsx FTP).
 * Usado por ExcelHandler (automático) y export manual filtrado.
 */

import { prisma } from '../index';
import { IVenta } from '../types';
import { excelTemplateService, VentaExcelRow } from './handlers/sale-created/utils/excel-template.service';
import {
    formatFechaVenta,
...
} from './handlers/sale-created/utils/excel-formatters';
import { EventContext } from './handlers/handler.interface';
...
/** Include Prisma para venta completa (Excel ERP). */
export const VENTA_EXCEL_INCLUDE = {
    cliente: {
        include: {
            usuarios: true,
        },
...
export class VentasExcelExportService {
    async getVentaCompleta(idVenta: number): Promise<IVenta> {
        ...
        return this.formatVentaRecord(venta);
    }

    formatVentaFromPrisma(venta: any): IVenta {
        return this.formatVentaRecord(venta);
    }

    private formatVentaRecord(venta: any): IVenta {
    ...
    async mapVentaToExcelRows(venta: IVenta, context: EventContext | { handlerData?: Record<string, unknown> }): Promise<VentaExcelRow[]> {
    ...
    async buildWorkbookBuffer(
        ventas: IVenta[],
        context: EventContext | { handlerData?: Record<string, unknown> } = { handlerData: {} },
    ): Promise<{ buffer: Buffer; rowsCount: number }> {
        const workbook = excelTemplateService.createTemplate();
        const allRows: VentaExcelRow[] = [];
        for (const venta of ventas) {
            const rows = await this.mapVentaToExcelRows(venta, context);
            allRows.push(...rows);
        }
        excelTemplateService.appendVentaRows(workbook, allRows, 4);
        return {
            buffer: excelTemplateService.writeWorkbookToBuffer(workbook),
            rowsCount: allRows.length,
        };
    }
}

export const ventasExcelExportService = new VentasExcelExportService();
