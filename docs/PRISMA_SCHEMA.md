...
Las relaciones con **productos** siguen siendo por códigos: `codi_categoria`, `codi_marca`, `codi_grupo`, `codi_impuesto`. Los productos usan `lista_precio_activa` para indicar qué precio está en uso (V/O/P/Q/E). La lista **E** (Precio especial) usa `precio_manual` y no se sobreescribe por sincronización CSV.
